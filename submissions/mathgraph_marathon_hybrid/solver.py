...
class MarathonCandidateRecorded(Exception):
    """Control signal: the official Marathon scorer must verify the candidate."""
...
def main():
    ...
    for index, problem in enumerate(problems):
        ...
        MARATHON_MESSAGES[:] = [{
            ...
            "budget": {
                "timeout_seconds": max(
                    1.0, remaining / max(1, len(problems) - index)
                )
            },
        }]
        try:
    ...
    # One-shot shared-budget fallback for deterministic residuals. Candidates
    # are never self-accepted or cached; the official Marathon scorer remains
    # the only authority.
    try:
    ...
    if call_llm is not None:
        residuals = [
            ...
            if problem.get("id") not in MARATHON_RECORDED
        ]
        residuals.sort(key=lambda problem: (
            len(problem.get("equation1", ""))
            + len(problem.get("equation2", "")),
            problem.get("id", ""),
        ))
        configuration = {
        ...
        for problem in residuals:
            if time.monotonic() + 15 >= started + budget:
                break
            prompt = (
                "Prove this equational implication for every magma in Lean 4.\n"
                "Source law: " + problem["equation1"] + "\n"
                "Target law: " + problem["equation2"] + "\n\n"
                "Your proof is inserted after `intro G _ h`. Introduce all "
                "target variables. Derive every projection, absorption, "
                "constancy, or diagonal lemma explicitly from specializations "
                "of h. Do not assume associativity or commutativity. Do not "
                "use sorry, admit, axioms, imports, or theorem declarations.\n"
                "Return only JSON: {\"proof\":\"<Lean tactic body>\"}"
            )
            try:
                response = call_llm(prompt, config=configuration)
            except Exception:
                continue
            if "error" in response:
                if "exhaust" in str(response.get("error", "")).lower():
                    break
                continue
            ...
            try:
                ...
                left = text.find("{") if isinstance(text, str) else -1
                right = text.rfind("}") if isinstance(text, str) else -1
            ...
            if (
                ...
                or any(token in proof for token in (
                    "sorry", "admit", "axiom", "theorem",
                    "def submission", "import ", "native_decide",
                ))
            ):
                continue
            body = "\n".join(
