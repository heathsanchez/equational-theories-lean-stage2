...
PROMPT = """You are the final proof-producing fallback for an equational
implication over an arbitrary magma in Lean 4.

Source law: {problem.equation1}
Target law: {problem.equation2}

The deterministic solver has exhausted direct instances, equality chains,
finite countermodels through order four, quotient matching, normalization, and
bounded unit superposition. The implication is therefore likely TRUE, but you
must produce a checkable Lean proof rather than merely classify it.

Your text is inserted after:
  intro G _ h
...
`Eq.symm`, and `Eq.trans` steps. Useful proofs often first derive a projection,
absorption, constancy, or diagonal lemma from two or more specializations of h.
Do not assume associativity, commutativity, or any magma property not derived
from h. Do not use `sorry`, `admit`, new axioms, imports, theorem declarations,
or native_decide.

Judge feedback from earlier attempts:
{history.attempts}

Solver round: {solver.round}
...
def extract_llm_proof(text):
    ...
    if (
        not proof
        or len(proof.encode("utf-8")) > 90000
...
def run_solo():
    ...
    # This is intentionally last among deterministic routes.
    ...
    # Verified hybrid fallback. Model output is only a candidate: Lean remains
    # the sole authority and failed attempts are fed back by the proxy.
