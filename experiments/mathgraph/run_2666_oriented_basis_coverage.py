...
spec = importlib.util.spec_from_file_location('sep18_helper_orientedbasis', HELPER)
...
def main():
    ...
    def H(args):
        ...
        return add(m.EqualityNode(m.substitute(sl,mp),m.substitute(sr,mp),'source instance',
                                  substitution=tuple((v,mp[v]) for v in vs),constructor='oriented-basis'),0)
    def R(t): return add(m.EqualityNode(t,t,'reflexivity',constructor='oriented-basis'),0)
    def S(i,d):
        p=nodes[i]; return add(m.EqualityNode(p.rhs,p.lhs,'symmetry',parents=(i,),constructor='oriented-basis'),d)
    def T(i,j,d):
        a,b=nodes[i],nodes[j]
        if a.rhs != b.lhs: return None
        return add(m.EqualityNode(a.lhs,b.rhs,'transitivity',parents=(i,j),constructor='oriented-basis'),d)
    def C(i,j,d):
        a,b=nodes[i],nodes[j]
        lhs=M(a.lhs,b.lhs); rhs=M(a.rhs,b.rhs)
        if m.term_size(lhs)>MAX_TERM_SIZE or m.term_size(rhs)>MAX_TERM_SIZE: return None
        i1=add(m.EqualityNode(M(a.lhs,b.lhs),M(a.rhs,b.lhs),'congruence on left child',parents=(i,),context=('left',b.lhs),constructor='oriented-basis'),d)
        if i1 is None: return None
        i2=add(m.EqualityNode(M(a.rhs,b.lhs),M(a.rhs,b.rhs),'congruence on right child',parents=(j,),context=('right',a.rhs),constructor='oriented-basis'),d)
    ...
    # Persistent oriented basis: both directions of every frozen source atom stay
    # eligible as continuation partners throughout search. This is generic with
    # respect to the basis and does not privilege a teacher milestone.
    oriented_basis=[]
    for i in atom_ids:
        oriented_basis.append(i)
        oriented_basis.append(S(i,0))
    oriented_basis=list(dict.fromkeys(oriented_basis))
    ...
    frontier=[first_level]; snapshots=[coverage(1)]; found=best.get((tl,tr))
    for r in range(2,ROUNDS+1):
        ...
        # Role-aware congruence continuation from the successful previous arm.
        for i in active:
        ...
        # New intervention: exact-chain composition with the persistent oriented basis
        # before any wider frontier chaining.
        seed_new=list(dict.fromkeys(new))
        for i in seed_new[:FRONTIER_CAP]:
            for j in oriented_basis:
                k=T(i,j,r)
                if k is not None and depth[k]==r: new.append(k)
                k=T(j,i,r)
                if k is not None and depth[k]==r: new.append(k)

        # Preserve symmetry and exact-chain continuation for newly developed states.
        ...
        snapshots.append(snap); print(json.dumps({k:v for k,v in snap.items() if k!='rows'},sort_keys=True),flush=True)
        if not frontier or len(nodes)>=MAX_NODES: break

    final=snapshots[-1]
    out={'schema':'mathgraph.2666-oriented-basis-coverage.v1','source_atoms':6,
         'persistent_oriented_basis':len(oriented_basis),'source_instantiation_disabled':True,
         'node_count':len(nodes),'found':found is not None,
         ...
         'promotion_gate': {'baseline_present':3,'passed': final['present'] > 3 or found is not None}}
    ...
    p=Path('experiments/mathgraph/results/2666-oriented-basis-coverage.json'); p.parent.mkdir(parents=True,exist_ok=True)
    ...
    print('SUMMARY',json.dumps({'found':out['found'],'replayed':out['replayed'],'nodes':out['node_count'],
          ...
          'first_missing':out['first_missing'],'promotion_gate':out['promotion_gate']},sort_keys=True),flush=True)
