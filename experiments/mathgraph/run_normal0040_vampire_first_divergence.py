...
def load_solver():
    spec = importlib.util.spec_from_file_location('mg_normal0040_divergence_solver', SOLVER)
...
def load_row(path):
    ...
    for i,row in enumerate(rows):
        rid=row.get('id') or row.get('problem_id') or row.get('name') or f'evaluation_normal_{i:04d}'
        if rid==RID:
            row=dict(row); row['id']=rid; return row
    raise RuntimeError(f'{RID} not found')

def extract_parents(tail):
    joined=','.join(tail)
    # Keep raw parent ids as evidence without trying to fully parse Vampire's inference annotations.
    m=re.search(r'inference\([^\[]*\[[^\]]*\],\s*\[([^\]]*)\]',joined,re.S)
    if not m: return []
    return re.findall(r'\b(?:f|c)_?\d+\b|\b\d+\b',m.group(1))

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--input',required=True); ap.add_argument('--output',required=True); args=ap.parse_args()
    m=load_solver(); row=load_row(args.input)
    ...
    engine=m.TargetGroundedRefutation(source,target,time.monotonic()+15.0,limits); found=engine.solve(); rigid=m.RigidSuperpositionModule()
    student=[]
    for c in engine.search.clauses:
        student.append((inline_engine_names(c.lhs,engine.reverse_constants),inline_engine_names(c.rhs,engine.reverse_constants)))
    ...
    for block in fof_blocks(proof):
        ...
        except Exception as e: eq=None; parse_failures.append({'id':fid,'error':type(e).__name__,'message':str(e)})
        ...
        rec={'id':fid,'inference':inf,'present':present,'exact':exact,'parents':extract_parents(tail),'lhs':m.render_term(ia),'rhs':m.render_term(ib),'lhs_size':m.term_size(ia),'rhs_size':m.term_size(ib),'raw_tail':tail}
        audited.append(rec)
        if first is None and not present: first=rec
    out={'schema':'mathgraph.normal0040-vampire-first-divergence.v1','id':RID,'equation1':row['equation1'],'equation2':row['equation2'],'solver_found':bool(found),'clauses':len(engine.search.clauses),'rounds':engine.search.rounds,'superpositions':engine.search.superpositions,'reductions':engine.search.reductions,'audited_steps':len(audited),'present_steps':sum(x['present'] for x in audited),'exact_steps':sum(x['exact'] for x in audited),'first_missing':first,'parse_failures':parse_failures,'steps':audited}
    Path(args.output).parent.mkdir(parents=True,exist_ok=True); Path(args.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_FIRST_DIVERGENCE',json.dumps({k:out[k] for k in ('solver_found','clauses','rounds','superpositions','reductions','audited_steps','present_steps','exact_steps','first_missing')},sort_keys=True),flush=True)
