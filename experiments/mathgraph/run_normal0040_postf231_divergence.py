...
def main():
 ...
 m=load(SOLVER,'mgpost231'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'hpost231')
 ...
 for block in h.fof_blocks(proof):
  ...
  if kind=='definition':
   ...
   continue
  if fid=='f231': after=True; continue
 ...
 out={'id':RID,'f95_seeded':bool(s95),'f123_seeded':bool(s123),'f148_seeded':bool(s148),'f150_seeded':bool(s150),'f196_seeded':bool(s196),'f217_seeded':bool(s217),'f229_seeded':bool(s229),'f231_seeded':bool(s231),'clauses_after_seeds':len(e.search.clauses),'audited_after_f231':len(audited),'present_after_f231':sum(x['present'] for x in audited),'first_missing_after_f231':first,'first_steps':audited[:8]}
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_POSTF231_DIVERGENCE',json.dumps(out,sort_keys=True),flush=True)
