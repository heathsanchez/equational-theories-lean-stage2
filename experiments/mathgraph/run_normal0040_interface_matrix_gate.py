...
PROTO=ROOT/'experiments/mathgraph/protocols/normal0040-interface-matrix-v1.json';OUT=ROOT/'experiments/mathgraph/results/normal0040-interface-matrix-gate.json';RID='evaluation_normal_0040'
...
def main():
 p=json.loads(PROTO.read_text());m=load(SOLVER,'mgmx');sym=load(SYM,'symmx');selfm=load(SELF,'selfmx');op=load(OPC,'opmx');op.selfmod=selfm;rhs=load(RHS,'rhsmx');rhs.selfm=selfm;ep=load(EP,'epmx');cut=load(CUT,'cutmx');rou=load(ROU,'roumx');rou.epmod=ep
  ...
  for side_name,pat in [('lhs',source[0]),('rhs',source[1])]:
   for path,sub in rou.paths(term):
    ...
    for vals in itertools.product(fillers,repeat=len(missing)):
     full=dict(mp);full.update(zip(missing,vals));it=rou.lifted_item(m,selfm,source,target,term,path,side_name,full,'mx')
     if not it:continue
     endpoint=it['schema'][1]
     if m.term_size(endpoint)>maxt:continue
     k=canon(m,endpoint)
     if k in seen:continue
     seen.add(k);desc=(side_name,pathshape(path),subst_pattern(m,full,vars_),bucket(md(m,endpoint,L)),bucket(md(m,endpoint,R)),shared_nontrivial(m,endpoint,oppshell));out.append({'endpoint':endpoint,'desc':desc,'side':side_name,'path':tuple(path),'subst':full})
     if len(out)>=cap:cache[ck]=out;return out
 ...
 def compat_item(a,b):return a['side']==b['side'] and pathshape(a['path'])==pathshape(b['path']) and rou.compatible(a['subst'],b['subst'])
   ...
   for tb in B[:p['bounds']['maximum_terms_per_class']]:
    for a in interactions(ta,'L'):
     for b in interactions(tb,'R'):
      if compat_item(a,b):return {'lhs_term':m.render_term(ta),'rhs_term':m.render_term(tb),'source_side':a['side'],'path_shape':pathshape(a['path']),'lhs_desc':repr(a['desc']),'rhs_desc':repr(b['desc'])}
  return None
 matrix=[];hits=[]
 for i,(_,A) in enumerate(classes['L']):
  ...
  matrix.append(row)
 out={'schema':'mathgraph.normal0040-interface-matrix.v1','id':RID,'protocol':p,'frozen_distance':d,'generation1':{'lhs_terms':len(fronts['L']),'rhs_terms':len(fronts['R']),'lhs_classes':len(classes['L']),'rhs_classes':len(classes['R'])},'compatibility_matrix':matrix,'compatible_cells':len(hits),'hits':hits,'decision':'GEN1_COMPATIBILITY_EXISTS' if hits else 'NO_GEN1_COMPATIBILITY_TRANSITION_REQUIRED'}
