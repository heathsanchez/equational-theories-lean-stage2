...
def main():
 ...
 m=load(SOLVER,'mgpost196'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'hpost196')
 ...
 for block in h.fof_blocks(proof):
  ...
  if kind=='definition':
   ...
   continue
  if fid=='f196': after=True; continue
 ...
 out={'id':RID,'f95_seeded':s95 is not None,'f123_seeded':s123 is not None,'f148_seeded':s148 is not None,'f150_seeded':s150 is not None,'f196_seeded':s196 is not None,'clauses_after_seeds':len(e.search.clauses),'audited_after_f196':len(audited),'present_after_f196':sum(x['present'] for x in audited),'first_missing_after_f196':first,'first_steps':audited[:8]}
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_POSTF196_DIVERGENCE',json.dumps(out,sort_keys=True),flush=True)
