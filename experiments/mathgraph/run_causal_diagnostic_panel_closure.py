...
def barm(m,row,seconds):
 e=engine(m,row,seconds);t=time.monotonic();r=e.search.solve();ok=replay(m,e,r)
 s=e.search
...
def garm(m,gate,row,seconds):
 e=engine(m,row,seconds);t=time.monotonic();r,s=gate.solve_given(m,e.search);ok=replay(m,e,r)
 out={'closure':float(ok),'elapsed':time.monotonic()-t}
...
def main():
 ...
 for cfg in CONFIGS:
  ...
  # Deterministic spread across each configuration, not hand-picked theorem IDs.
  if not ds:continue
  idx=sorted(set(min(len(ds)-1,round(i*(len(ds)-1)/max(1,PER_CFG-1))) for i in range(PER_CFG)))
  for i in idx:selected.append((cfg,ds[i]))
 records=[]
 for n,(cfg,row) in enumerate(selected):
  t0=time.monotonic();s=static(m,row);bs,bshort=barm(m,row,SHORT);gs,gshort=garm(m,gate,row,SHORT)
  # Long arms are teacher-forced regardless of short result: this prevents the old controller from creating labels.
  bl,blong=barm(m,row,LONG);gl,glong=garm(m,gate,row,LONG)
 ...
 static_fs=sorted(f for f in rows[0]['x'] if f.startswith('s.')) if rows else [];panel_fs=sorted(rows[0]['x']) if rows else []
 runs=[]
 ...
 out={'schema':'mathgraph.causal-diagnostic-panel-closure.v1','protocol':{'short_seconds':SHORT,'long_seconds':LONG,'teacher_forced_long_arms':True,'no_external_proof_trace':True,'problem_disjoint':True},'cases':len(records),'label_counts':dict(counts),'kept_labels':sorted(keep),'valid_splits':len(runs),'median_static_macro':round(med('static_macro'),4),'median_panel_macro':round(med('panel_macro'),4),'median_gain':round(med('gain'),4),'all_id_disjoint':all(r['id_overlap']==0 for r in runs),'records':records,'runs':runs}
 out['gates']={'G1_two_or_more_labels':len(keep)>=2,'G2_at_least_15_valid_splits':len(runs)>=15,'G3_disjoint':out['all_id_disjoint'],'G4_panel_macro_ge_060':out['median_panel_macro']>=.60,'G5_panel_gain_ge_010':out['median_gain']>=.10};out['closure_pass']=all(out['gates'].values())
