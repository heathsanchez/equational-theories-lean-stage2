...
def main():
 ...
 m=load(SOLVER,'mg196'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h196')
 ...
 W=h.extract_wanted(proof,target[2],m,('f20','f27','f81','f95','f123','f126','f130','f148','f150','f196'))
 def cover(eq):
 ...
 s196=cp(s148,s150,W['f196']) if s148 and s150 else None; out['f196_generated']=bool(s196); out['f196_seed_added']=False
 if s196: out['f196_seed_added']=bool(e.search.add_clause(s196))
 found=e.solve() if s196 else None; out['seeded_found']=bool(found); out['seeded_replay_ok']=False; out['proof_nodes']=None
 if found:
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_F196_ATTACHMENT',json.dumps(out,sort_keys=True),flush=True)
