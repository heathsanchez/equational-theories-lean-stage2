...
def main():
 ...
 m=load(SOLVER,'mgpost244'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'hpost244')
 ...
 s231=cp(s229,c126,W['f231']) if s229 and c126 else None
 if s231: e.search.add_clause(s231)
 s244=cp(s148,s231,W['f244']) if s148 and s231 else None
 ...
 for block in h.fof_blocks(proof):
  ...
  if kind=='definition':
   ...
   continue
  if fid=='f244': after=True; continue
 ...
 out={'id':RID,'f95_seeded':bool(s95),'f123_seeded':bool(s123),'f148_seeded':bool(s148),'f150_seeded':bool(s150),'f196_seeded':bool(s196),'f217_seeded':bool(s217),'f229_seeded':bool(s229),'f231_seeded':bool(s231),'f244_seeded':bool(s244),'clauses_after_seeds':len(e.search.clauses),'audited_after_f244':len(audited),'present_after_f244':sum(x['present'] for x in audited),'first_missing_after_f244':first,'first_steps':audited[:8]}
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_POSTF244_DIVERGENCE',json.dumps(out,sort_keys=True),flush=True)
