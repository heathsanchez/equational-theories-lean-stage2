...
def main():
 ...
 m=load(SOLVER,'mg259'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h259')
 ...
 W=h.extract_wanted(proof,target[2],m,('f17','f18','f19','f20','f27','f81','f95','f123','f126','f130','f148','f150','f196','f217','f229','f231','f244','f258','f259'))
 def cover(eq):
 ...
 c81=cover(W['f81']); s95=cp(c81,c81,W['f95']) if c81 else None; out['f95_generated']=bool(s95)
 if s95: e.search.add_clause(s95)
 c27=cover(W['f27']); s123=cp(c27,s95,W['f123']) if c27 and s95 else None; out['f123_generated']=bool(s123)
 if s123: e.search.add_clause(s123)
 c20=cover(W['f20']); c126=cover(W['f126']); s148=cp(c20,c126,W['f148']) if c20 and c126 else None; out['f148_generated']=bool(s148)
 if s148: e.search.add_clause(s148)
 c130=cover(W['f130']); s150=cp(c130,c130,W['f150']) if c130 else None; out['f150_generated']=bool(s150)
 ...
 c19=cover(W['f19']); s217=cp(c19,s196,W['f217']) if c19 and s196 else None; out['f217_generated']=bool(s217)
 if s217: e.search.add_clause(s217)
 c18=cover(W['f18']); s229=cp(c18,s217,W['f229']) if c18 and s217 else None; out['f229_generated']=bool(s229)
 if s229: e.search.add_clause(s229)
 s231=cp(s229,c126,W['f231']) if s229 and c126 else None; out['f231_generated']=bool(s231)
 if s231: e.search.add_clause(s231)
 s244=cp(s148,s231,W['f244']) if s148 and s231 else None; out['f244_generated']=bool(s244)
 if s244: e.search.add_clause(s244)
 c17=cover(W['f17']); s258=cp(c17,s244,W['f258']) if c17 and s244 else None; out['f258_generated']=bool(s258)
 if s258: e.search.add_clause(s258)
 s259=cp(s258,s217,W['f259']) if s258 and s217 else None; out['f259_generated']=bool(s259); out['f259_seed_added']=False
 if s259: out['f259_seed_added']=bool(e.search.add_clause(s259))
 found=e.solve() if s259 else None; out['seeded_found']=bool(found); out['seeded_replay_ok']=False; out['proof_nodes']=None
 if found:
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_F259_ATTACHMENT',json.dumps(out,sort_keys=True),flush=True)
