#!/usr/bin/env python3
"""Frozen broader source-distinct transfer audit for the generic given-clause controller."""
...
OUT=ROOT/'experiments/mathgraph/results/given-clause-transfer40-gate.json'
...
# Frozen before outcome inspection: ten spread TRUE rows per config.
...
def main():
 m=load(SOLVER,'mg_transfer40');gate=load(ROOT/'experiments/mathgraph/run_given_clause_saturation_gate.py','given_gate_transfer40')
 rows=[]
 for cfg in CONFIGS:
 ...
 out={'schema':'mathgraph.given-clause-transfer40-gate.v1','seconds':SECONDS,'indices':INDICES,'rows':[]}
 ...
 if out['summary']['regressions']:raise SystemExit(2)
