...
def load_solver():
    ...
    spec=importlib.util.spec_from_file_location('mg796sep',p); m=importlib.util.module_from_spec(spec); sys.modules[spec.name]=m; spec.loader.exec_module(m)
    return td,m
...
def statements(text):
    ...
    for line in text.splitlines():
        z=line.strip()
        if not active and (z.startswith('fof(') or z.startswith('cnf(')):
            active=True; cur=[]; depth=0
        if active:
            cur.append(z); depth += z.count('(')-z.count(')')
            if depth==0 and z.endswith('.'):
                out.append(' '.join(cur)); active=False
    return out
...
def first_vampire_superposition(m,r):
    ...
    for st in statements(run.stdout+run.stderr):
        got=parse_stmt(st)
        if not got: continue
        formula,inf=got
        if inf=='superposition':
            k=formula_key(formula)
            if k is not None: return source,target,k,formula
    raise RuntimeError('no Vampire superposition equality parsed')
...
def trace_target(m,source,target,wanted,seconds=20.0):
    ...
    hit={'raw':False,'post_interreduce':False,'best_rank':None,'topk':False,'add_clause':False,'passive':False,'selected':False}
    while passive and given<1024 and not search.expired():
        ...
        if clause_key(selected)==wanted: hit['selected']=True
        reduced=search.interreduce(selected,rules); selected=reduced; active.append(selected); given+=1
        rules=[]
        for c in active:
            q=search.orient(c)
            if q is not None: rules.append(q)
                ...
                for oside,outer in enumerate(oriented_variants(m,bo)):
                    for iside,inner in enumerate(oriented_variants(m,bi)):
                        for path in m.nonvariable_positions(outer.lhs,maximum_depth=search.limits['maximum_depth'],include_root=True):
                            ...
                            if clause_key(q)==wanted: hit['raw']=True
                            qr=search.interreduce(q,rules)
                            if clause_key(qr)==wanted: hit['post_interreduce']=True
                            proposals.append((search.target_score(qr),qr))
        proposals.sort(key=lambda x:x[0])
        ranks=[i for i,(_,q) in enumerate(proposals) if clause_key(q)==wanted]
        if ranks:
            mr=min(ranks); hit['best_rank']=mr if hit['best_rank'] is None else min(hit['best_rank'],mr)
            if mr < search.limits['new_clauses_per_round']: hit['topk']=True
        for _,q in proposals[:search.limits['new_clauses_per_round']]:
            is_wanted=clause_key(q)==wanted
            if search.add_clause(q):
                if is_wanted: hit['add_clause']=True
                passive.append(q); age[id(q)]=next_age; next_age+=1
        if any(clause_key(c)==wanted for c in passive): hit['passive']=True
        new=[]; seen=set()
        for c in passive:
            if search.expired(): break
            red=search.interreduce(c,rules); c=red
            names={}; a=(m.alpha_canonical_term(c.lhs,names),m.alpha_canonical_term(c.rhs,names)); names={}; b=(m.alpha_canonical_term(c.rhs,names),m.alpha_canonical_term(c.lhs,names)); k=min(a,b)
            if k in seen: continue
            seen.add(k); new.append(c)
        passive=new
    hit['given']=given; hit['generated']=search.generated; hit['superpositions']=search.superpositions
    return hit


def main():
    td,m=load_solver(); out=[]
    try:
        for r in rows():
            source,target,wanted,formula=first_vampire_superposition(m,r)
            hit=trace_target(m,source,target,wanted)
            rec={'id':r['id'],'vampire_formula':formula,'pipeline':hit}; out.append(rec)
            print('SEPARATOR',json.dumps(rec,sort_keys=True),flush=True)
    finally: td.cleanup()
    p=ROOT/'experiments/mathgraph/results/residual3-pipeline-separator.json'; p.parent.mkdir(parents=True,exist_ok=True); p.write_text(json.dumps({'rows':out},indent=2,sort_keys=True)+'\n')
