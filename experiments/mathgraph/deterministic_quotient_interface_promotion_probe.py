#!/usr/bin/env python3
"""Deterministic quotient-interface promotion probe for fresh Stage-2 residuals.

Unlike the earlier spectrum probes, neither the pre-quotient development nor the
compiled-interface census uses elapsed wall clock as a selection rule.  Each
phase is bounded by explicit critical-pair/candidate counts.  Time is only a
large emergency ceiling enforced by the surrounding workflow.
"""
...
def main():
    ...
    ap.add_argument('--round-attempts',type=int,default=12000); ap.add_argument('--census-budget',type=int,default=256)
    ...
    round_stats=[]
    for rnd in range(3):
        rules=s.rules(); snap=list(rules); props=[]; attempts=0; generated=0
        stop=False
        for oi,o in enumerate(snap):
            ...
            for ii,i in enumerate(snap):
                ...
                for path in m.nonvariable_positions(o.lhs,maximum_depth=12,include_root=True):
                    if attempts>=a.round_attempts:
                        stop=True; break
                    attempts+=1
                    c=s.critical_pair(o,i,oi,ii,path)
                    if c is None: continue
                    generated+=1; c=s.interreduce(c,rules); props.append((s.target_score(c),c))
                    if len(props)>=128:
        ...
        if props:
            ...
            for _,q in props:
                if s.add_clause(q): s.superpositions+=1; added+=1
                if added>=64: break
        round_stats.append({'round':rnd,'attempts':attempts,'generated':generated,'clauses':len(s.clauses),'superpositions':s.superpositions})
    ...
    x=('var','x'); xx=('op',x,x); rules=s.rules(); exhausted=True
    for oi,o in enumerate(rules):
        if census>=a.census_budget: exhausted=False; break
        for ii,i in enumerate(rules):
            ...
            for path in m.nonvariable_positions(o.lhs,maximum_depth=12,include_root=True):
                ...
                raw=canon(c.lhs,c.rhs); pe,ps=setup(raw); pn,pr=ps.compile(c)
                ...
                rec={'lhs':k[0],'rhs':k[1],'future_size':len(fs),'proof_nodes':len(pn),'raw_lhs':m.render_term(c.lhs),'raw_rhs':m.render_term(c.rhs),'census_index':census}
                prev=spectrum.get(k)
    ...
    if chosen is not None:
        te,ts=setup(target); added=ts.add_clause(chosen)
    ...
    print('DETERMINISTIC_QUOTIENT_PROMOTION '+json.dumps({'id':row['id'],'round_attempt_budget':a.round_attempts,'rounds':round_stats,'census_budget':a.census_budget,'census':census,'enumeration_exhausted':exhausted,'replayed':replayed,'projected':projected,'idempotence':chosen_info,'interfaces':vals,'target':target_rec},sort_keys=True),flush=True)
