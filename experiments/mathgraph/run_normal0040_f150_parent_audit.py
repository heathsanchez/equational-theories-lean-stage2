...
def load_helper():
    ...
    spec=importlib.util.spec_from_file_location('mg_f150_helpers',p)
...
def main():
    ...
    h=load_helper(); trace=json.load(urllib.request.urlopen(TRACE_URL)); proof=next(r['proof'] for r in trace['rows'] if r['id']==RID)
    rec={}
    ...
    target=rec.get('f150')
    out={'id':RID,'f150':target,'parents':{p:rec.get(p) for p in (target or {}).get('parents',[])}}
    Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
    print('NORMAL0040_F150_PARENT_AUDIT',json.dumps(out,sort_keys=True),flush=True)
