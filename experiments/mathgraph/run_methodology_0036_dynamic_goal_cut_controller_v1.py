...
def select_raw(m,s,target,cands):
    ...
    return scored[0][2], scored[0][0], sum(1 for d,_,_ in scored if d < (state_distance(m,s,target)[0] or 999999))
...
def run_arm(m,source,target,cfg,mode,max_steps,pool_limit):
    ...
    rows=[]; best=0 if st.get('connected') else st['cross_distance']; closure_step=None; stall=False
    for step in range(1,max_steps+1):
        pre,st=tour.cut.component_state(m,s,target,40),None
        # component_state returns dict; normalize
        pre_st=pre
        if pre_st is None: break
        ...
        if mode=='raw':
            chosen,pred,strict=select_raw(m,s,target,cands)
        else:
            chosen,pred,strict=select_target(cands,pre_d)
        ...
        rows.append({
            'step':step,'pre_distance':pre_d,'post_distance':post_d,
            'candidate_pool':len(cands),'strict_contractors':strict,
            'predicted_selected_distance':pred,'selected_added':added,
            'selected':tour.public_candidate(m,chosen),
            'lhs_component_size':None if post_st is None else post_st.get('lhs_size'),
            'rhs_component_size':None if post_st is None else post_st.get('rhs_size'),
            'connected':bool(post_st and post_st.get('connected')),
        ...
        if strict==0 or post_d is None or post_d>=pre_d:
            # Controller has no expressible strict contractor or selected move failed to contract.
            stall=True; break
    return {'rows':rows,'best_distance':best,'closure_step':closure_step,'stalled':stall,'admitted_reentries':admitted,'reentry_edges':edges,'initial_lhs_size':st.get('lhs_size'),'initial_rhs_size':st.get('rhs_size'),'initial_distance':0 if st.get('connected') else st.get('cross_distance')}
...
def main():
    ...
    # independent parent reconstruction for measurement gate
