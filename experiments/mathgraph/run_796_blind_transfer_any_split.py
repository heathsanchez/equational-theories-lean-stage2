#!/usr/bin/env python3
"""Run the frozen 0040 mechanism on an arbitrary Stage 2 evaluation row.

This wrapper changes only the selected problem id and row loader. It does not
inject proof IDs, lemmas, target-specific intermediates, or per-problem heuristics.
"""
...
if not a.id.startswith('evaluation_'):
    raise SystemExit('blind transfer requires an evaluation_* id')
