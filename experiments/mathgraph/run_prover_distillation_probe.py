...
superposition/demodulation equality absent from the student's reachable clause
surface.  This is diagnostic only: it never changes solver.py or emits answers.
"""
...
def formula_eq(formula):
    ...
    for i,c in enumerate(s):
        ...
        elif c=='=' and d==0 and not(i and s[i-1]=='!'):
...
def teacher_steps(m,proof,target_vars):
    ...
    for block in fof_blocks(proof):
        ...
        a=map_rigid(inline_defs(a,defs),target_vars); b=map_rigid(inline_defs(b,defs),target_vars)
        out.append((fid,inf,a,b))
    return out

def feature(m,a,b,inf):
    va=len(m.RigidSuperpositionModule.term_variables(a)|m.RigidSuperpositionModule.term_variables(b))
    sa,sb=m.term_size(a),m.term_size(b)
    bare=int(a[0]=='var' or b[0]=='var')
    collapse=int(bare and va<=1)
...
def main():
    ...
    m=load_solver(); rows=json.load(open(args.input)); rows=sorted(rows,key=lambda r:r['id'])[:args.limit]; rigid=m.RigidSuperpositionModule(); recs=[]; buckets=Counter()
    for row in rows:
        source=m.parse_equation(row['equation1']); target=m.parse_equation(row['equation2'])
        theorem,proof=run_vampire(vampire_problem(m,source,target),2)
        ...
        cs=set()
        for c in engine.search.clauses: cs.add(sig(rigid,inline_engine(c.lhs,engine.reverse_constants),inline_engine(c.rhs,engine.reverse_constants)))
        steps=teacher_steps(m,proof,target[2]); first=None; present=0
        for fid,inf,a,b in steps:
            ok=sig(rigid,a,b) in cs
            if ok: present+=1
            elif first is None: first={'fid':fid,'lhs':m.render_term(a),'rhs':m.render_term(b),**feature(m,a,b,inf)}
        if first: buckets[(first['inference'],first['bare_side'],first['collapse_like'],first['variables'])]+=1
        recs.append({'id':row['id'],'teacher':True,'student_found':bool(found),'teacher_steps':len(steps),'present_teacher_steps':present,'first_missing':first,'student_clauses':len(engine.search.clauses)})
        print(row['id'],'student',bool(found),'teacher_steps',len(steps),'first',first,flush=True)
