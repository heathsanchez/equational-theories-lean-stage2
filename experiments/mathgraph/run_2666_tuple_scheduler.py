...
spec=importlib.util.spec_from_file_location('sep18_helper_tuple',HELPER)
...
def main():
  ...
  limits={'max_term_size':35,'max_pool_terms':220,'max_core_terms':len(core),'max_source_attempts':900000,'max_source_edges':60000,'max_graph_edges':120000,'max_derivation_nodes':130000,'max_congruence_rounds':5}
  s=m.EqualitySearch(source,target,time.monotonic()+75,limits=limits); s.initial_pool=tuple(pool)
  sv=source[2]; attempts=0; tuple_attempts=0; tuple_added=0

  # Tuple-level fairness intervention: retain a quota for substitutions that contain
  # one rare constructor-family term, and enumerate the remaining coordinates over
  # the compact target/base vocabulary. This changes only scheduling, not terms/rules.
  rare=[]
  for t in core:
    if t in squares or t in attachments:
      rare.append(t)
  anchors=[]
  for t in base + core[:16]:
    if t not in anchors: anchors.append(t)
  anchors=anchors[:18]
  for rare_term in rare:
    for rare_pos in range(len(sv)):
      other=[i for i in range(len(sv)) if i!=rare_pos]
      for fills in product(anchors,repeat=len(other)):
        vals=[None]*len(sv); vals[rare_pos]=rare_term
        for p,val in zip(other,fills): vals[p]=val
        before=s.graph_edges; s.add_source_substitution(vals); attempts+=1; tuple_attempts+=1
        if s.graph_edges>before: tuple_added+=1
        if attempts>=s.max_source_attempts or s.graph_edges>=s.max_source_edges or s.expired(): break
      if attempts>=s.max_source_attempts or s.graph_edges>=s.max_source_edges or s.expired(): break
    if attempts>=s.max_source_attempts or s.graph_edges>=s.max_source_edges or s.expired(): break

  # Preserve the previous target-guided arm after the tuple quota.
  ...
  for _ in range(5):
    ...
    before=len(s.nodes); s.add_congruence_round(core[:60],first); first=before; root=s.shortest_path()

  out={'schema':'mathgraph.2666-tuple-scheduler.v1','pool':len(pool),'core':len(core),'rare_terms':len(rare),'anchors':len(anchors),'tuple_attempts':tuple_attempts,'tuple_added':tuple_added,'attempts_total':attempts,'teacher_atoms_present':sum(r['present'] for r in rows),'teacher_atoms_total':6,'rows':rows,'graph_edges':s.graph_edges,'nodes':len(s.nodes),'exhaustion':s.exhaustion,'found':root is not None,'replayed':False}
  if root is not None:
    try: out['replayed']=bool(m.replay_dag(source,s.nodes,root,maximum_term_size=35,maximum_nodes=130000))
    except Exception as e: out['replay_error']=type(e).__name__+': '+str(e)
    out['proof']=h.proof_summary(s.nodes,root)
  p=Path('experiments/mathgraph/results/2666-tuple-scheduler.json'); p.parent.mkdir(parents=True,exist_ok=True); p.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print(json.dumps(out,sort_keys=True))
