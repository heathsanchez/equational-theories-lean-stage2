...
def main():
    ...
    seen=set(); spectrum={}; recipes={}; census=replayed=projected=0; rules=s.rules(); s.deadline=time.monotonic()+12.0
    for oi,o in enumerate(rules):
        for ii,i in enumerate(rules):
            for path in m.nonvariable_positions(o.lhs,maximum_depth=12,include_root=True):
                ...
                rec={'lhs':k[0],'rhs':k[1],'proof_nodes':len(pn),'raw_lhs':m.render_term(c.lhs),'raw_rhs':m.render_term(c.rhs)}
                prev=spectrum.get(k)
    ...
    def rank(k):
        l,r=m.parse_equation(k[0]+' = '+k[1])[:2]
        return (m.term_size(l)+m.term_size(r),spectrum[k]['proof_nodes'],k[0],k[1])
    ordered=sorted(spectrum,key=rank); bundles=[]
    for n in (4,8,16,32):
        keys=ordered[:n]; te,ts=setup(target,12.0); added=[]
        for k in keys: added.append(bool(ts.add_clause(recipes[k])))
        tq=ts.collapse_proof() or ts.target_proof(ts.rules()) or ts.solve()
        out={'k':n,'added':sum(1 for x in added if x),'found':tq is not None,'replay':False,'laws':[spectrum[k] for k in keys]}
        if tq is not None:
            q=te.inline_recipe(tq)
            if (q.lhs,q.rhs)==(target[1],target[0]): q=m.Recipe(q.rhs,q.lhs,'symmetry',(q,))
            if (q.lhs,q.rhs)==target[:2]:
                nn,rr=ts.compile(q); out['replay']=m.replay_dag(source,nn,rr,maximum_term_size=320,maximum_nodes=70000); out['proof_nodes']=len(nn)
        bundles.append(out)
        if out['replay']: break
    result={'id':row['id'],'elapsed':round(time.monotonic()-t0,4),'pre_trace':pre,'census':census,'replayed':replayed,'projected':projected,'interfaces':len(spectrum),'bundles':bundles,'recovered':any(x['replay'] for x in bundles)}
    print('SOURCE_ONLY_LAWBOOK_COMPOSITION '+json.dumps(result,sort_keys=True),flush=True)
    if not result['recovered']: raise SystemExit(2)
