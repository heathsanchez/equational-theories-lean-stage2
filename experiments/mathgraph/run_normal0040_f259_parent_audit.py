...
def main():
 ...
 for b in fof_blocks(proof):
  ...
  if not m or m.group(1)!='f259': continue
  mi=re.search(r'inference\(([^,\]]+).*?\[([^\]]+)\]',b,re.S)
  parents=[]
  if mi:
   parents=re.findall(r'f\d+',mi.group(2))
  rec={'inference':mi.group(1) if mi else '', 'parents':parents, 'block':b}
 out={'id':RID,'f259':rec}
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2)+'\n'); print('NORMAL0040_F259_PARENT_AUDIT',json.dumps(out,sort_keys=True),flush=True)
