...
def main():
    ...
    def relational(q1,q2,cap=12):
        nonlocal calls_pair, exact_child
        out=[]; ss=set()
    ...
    # Rank initial pair states by genuinely pair-only novelty.
    states=[]
    for i in range(len(cand)):
        ...
        if exact_child is not None: break
    states.sort(key=lambda z:(-z[0],z[1]))
    frontier=states[:4]
    trace=[]

    # Keep product states intact for two recursive generations.  Each state
    # produces pair-only children; the next frontier consists of pairs of those
    # children, never flattened into the ordinary clause scheduler.
    ...
    result={'id':row['id'],'pre_trace':pre,'candidates':len(cand),'initial_pair_states':len(states),'initial_max_novelty':max([x[0] for x in states],default=0),'recursive_trace':trace,'individual_calls':calls_ind,'pair_calls':calls_pair,'exact_child':exact_child is not None,'exact_child_replay':False}
