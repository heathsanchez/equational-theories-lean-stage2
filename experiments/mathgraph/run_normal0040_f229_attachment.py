...
def main():
 ...
 m=load(SOLVER,'mg229'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h229')
 ...
 c19=cover(W['f19']); out['f19_cover']=bool(c19); s217=cp(c19,s196,W['f217']) if c19 and s196 else None; out['f217_generated']=bool(s217)
 if s217: e.search.add_clause(s217)
 c18=cover(W['f18']); out['f18_cover']=bool(c18); s229=cp(c18,s217,W['f229']) if c18 and s217 else None; out['f229_generated']=bool(s229); out['f229_seed_added']=False
 if s229: out['f229_seed_added']=bool(e.search.add_clause(s229))
 found=e.solve() if s229 else None; out['seeded_found']=bool(found); out['seeded_replay_ok']=False; out['proof_nodes']=None
 if found:
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_F229_ATTACHMENT',json.dumps(out,sort_keys=True),flush=True)
