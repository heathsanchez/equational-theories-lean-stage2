#!/usr/bin/env python3
"""Registered-workflow carrier for the MSI provenance-edge ablation.

The registered alpha workflow supplies the historical CLI. This launcher uses
that carrier only to execute the frozen representation ablation on the remaining
order-5 residual 0042. Search depth and budgets are unchanged; only continuation
identity/applicability retention is varied. No proof IDs, hidden traces, named
...
RUNNER = ROOT / 'experiments/mathgraph/run_800_msi_provenance_edge_ablation.py'
...
def main():
    ...
    raw = ROOT / 'experiments/mathgraph/results/800-evaluation_order5_0042-msi-provenance-edge-ablation.json'
    ...
    print('MSI_PROVENANCE_EDGE_START', RID, flush=True)
    rc = subprocess.call(cmd, cwd=ROOT)
    rec = {'schema':'mathgraph.msi-provenance-edge-ablation-carrier.v1','id':RID,'returncode':rc,
           ...
           'experiment':'frozen-provenance-edge-ablation'}
    if raw.exists():
    ...
    print('MSI_PROVENANCE_EDGE_DONE', json.dumps(rec, sort_keys=True), flush=True)
    raise SystemExit(rc)
