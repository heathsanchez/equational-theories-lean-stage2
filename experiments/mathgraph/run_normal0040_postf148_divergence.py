...
def main():
 ...
 m=load_mod(SOLVER,'mg_postf148_solver'); h=load_mod(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'mg_postf148_helpers')
 ...
 e=m.TargetGroundedRefutation(source,target,time.monotonic()+15.0,limits); e.solve(); r=m.RigidSuperpositionModule()
 proof=next(x['proof'] for x in json.load(urllib.request.urlopen(TRACE_URL))['rows'] if x['id']==RID)
 w=h.extract_wanted(proof,target[2],m,('f20','f27','f81','f95','f123','f126','f148'))
 def fc(eq):
  ...
  return None
 def cp(a,b,g):
  for aa in (a,m.Recipe(a.rhs,a.lhs,'symmetry',(a,))):
   for bb in (b,m.Recipe(b.rhs,b.lhs,'symmetry',(b,))):
    for o,i in ((aa,bb),(bb,aa)):
     for pth in r.nonvariable_positions(o.lhs,maximum_depth=limits['maximum_depth'],include_root=True):
      p=e.search.critical_pair(o,i,0,1,pth)
      if p is None: continue
      x=h.inline_engine_names(p.lhs,e.reverse_constants); y=h.inline_engine_names(p.rhs,e.reverse_constants)
      if alpha_sig(r,x,y)==alpha_sig(r,*g): return p
  return None
 s95=s123=s148=None; c81=fc(w['f81'])
 if c81: s95=cp(c81,c81,w['f95'])
 if s95:
  e.search.add_clause(s95); c27=fc(w['f27']); s123=cp(c27,s95,w['f123']) if c27 else None
 if s123:
  e.search.add_clause(s123); c20=fc(w['f20']); c126=fc(w['f126']); s148=cp(c20,c126,w['f148']) if c20 and c126 else None
 ...
 for block in h.fof_blocks(proof):
  ...
  if kind=='definition':
   ...
   continue
  if fid=='f148': after=True; continue
  ...
  exact=any(alpha_sig(r,aa,bb)==alpha_sig(r,sa,sb) for sa,sb in student); present=exact or any(covers(r,sa,sb,aa,bb) for sa,sb in student)
 ...
 out={'id':RID,'f95_seeded':s95 is not None,'f123_seeded':s123 is not None,'f148_seeded':s148 is not None,'clauses_after_seeds':len(e.search.clauses),'audited_after_f148':len(audited),'present_after_f148':sum(x['present'] for x in audited),'first_missing_after_f148':first,'first_steps':audited[:8]}
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_POSTF148_DIVERGENCE',json.dumps(out,sort_keys=True),flush=True)
