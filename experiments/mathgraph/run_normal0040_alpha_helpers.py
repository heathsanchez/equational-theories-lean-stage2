...
def load_row(path,rid):
    rows=[json.loads(x) for x in Path(path).read_text().splitlines() if x.strip()]
    for i,r in enumerate(rows):
        rr=r.get('id') or f'evaluation_normal_{i:04d}'
        if rr==rid:
            q=dict(r); q['id']=rr; return q
    raise RuntimeError('row missing')

def extract_wanted(proof,target_vars,m,ids):
    defs={}; wanted={}
    for block in fof_blocks(proof):
        ...
        if kind=='definition':
            ...
            continue
        if fid in ids:
            wanted[fid]=(map_rigids(inline_defs(x,defs),target_vars),map_rigids(inline_defs(y,defs),target_vars))
    return wanted
