...
def load_helper():
    ...
    spec=importlib.util.spec_from_file_location('mg_f148_helpers',p)
...
def main():
    ...
    target=rec.get('f148')
    out={'id':RID,'f148':target,'parents':{p:rec.get(p) for p in (target or {}).get('parents',[])}}
    Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
    print('NORMAL0040_F148_PARENT_AUDIT',json.dumps(out,sort_keys=True),flush=True)
