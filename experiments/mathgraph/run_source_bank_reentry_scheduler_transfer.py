...
def run_one(rid,path,m,h):
 ...
 out={'id':rid,'policy':'pair-fair-plus-source-bank-reentry','warm_found':bool(warm),'oracle_free':True}
 if warm:
 ...
 for rnd in range(1,9):
  ...
  # Residual-derived fix: Vampire's shared missing bridges are source-law outer superpositions
  # with already-retained clauses (0036 f6×f35 -> f38; 0196 f6×f27 -> f30).
  reentry_raw=[]; reentry_attempts=0
  if fair_complete:
   for b in bank:
    for s in source_schemas[:8]:
     if time.monotonic()>=deadline: break
     reentry_raw.extend(directed_outer(s,b,cap=4)); reentry_attempts+=1
    if time.monotonic()>=deadline: break
  ...
  out['round_stats'].append({'round':rnd,'frontier_in':len(frontier),'bank':len(bank),'fair_complete':fair_complete,'pair_attempts':pair_attempts,'source_bank_reentry_attempts':reentry_attempts,'source_bank_reentry_raw':len(reentry_raw),'raw':len(raw),'selected':len(ranked),'added':len(new),'clauses':len(e.search.clauses)})
  frontier=new
...
def main():
 ...
 m=load(SOLVER,'mg_source_bank_reentry'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h_source_bank_reentry'); rows=[]
 for rid in CASES:
  path=args.normal_input if 'normal_' in rid else args.hard_input; res=run_one(rid,path,m,h); rows.append(res); print('SOURCE_BANK_REENTRY_CASE',json.dumps(res,sort_keys=True),flush=True)
 out={'policy':'pair-fair-plus-source-bank-reentry','rows':rows,'solved':sum(bool(x.get('found')) for x in rows),'intervention_solved':sum(bool(x.get('found')) and x.get('phase')=='finish' for x in rows),'replay_ok':sum(bool(x.get('replay_ok')) for x in rows)}
 Path(args.output).parent.mkdir(parents=True,exist_ok=True); Path(args.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('SOURCE_BANK_REENTRY_SUMMARY',json.dumps({k:out[k] for k in ('solved','intervention_solved','replay_ok')},sort_keys=True),flush=True)
