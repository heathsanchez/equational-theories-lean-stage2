...
def main():
 ...
 for rid,(src_id,bridge_id,want_id) in CASES.items():
  inp=a.normal_input if 'normal_' in rid else a.hard_input; row=h.load_row(inp,rid); source=m.parse_equation(row['equation1']); target=m.parse_equation(row['equation2'])
  ...
  hits_outer=[]; hits_reverse=[]
  for s in matches.get(src_id,[]):
   for b in matches.get(bridge_id,[]):
    a=directed(s,b); z=directed(b,s)
    hits_outer += [p for p in a if key(p)==keys.get(want_id)]
    hits_reverse += [p for p in z if key(p)==keys.get(want_id)]
  out.update(source_outer_hits=len(hits_outer),bridge_outer_hits=len(hits_reverse),source_outer_total=sum(len(directed(s,b)) for s in matches.get(src_id,[]) for b in matches.get(bridge_id,[])),bridge_outer_total=sum(len(directed(b,s)) for s in matches.get(src_id,[]) for b in matches.get(bridge_id,[])))
  rows.append(out); print('OUTER_INNER_ROLE_AUDIT',json.dumps(out,sort_keys=True),flush=True)
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps({'rows':rows},indent=2,sort_keys=True)+'\n')
