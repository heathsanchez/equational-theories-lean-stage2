...
def main():
 ...
 m=load(SOLVER,'mg258'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h258')
 ...
 W=h.extract_wanted(proof,target[2],m,('f17','f18','f19','f20','f27','f81','f95','f123','f126','f130','f148','f150','f196','f217','f229','f231','f244','f258'))
 def cover(eq):
 ...
 c20=cover(W['f20']); c126=cover(W['f126']); out['f126_cover']=bool(c126); s148=cp(c20,c126,W['f148']) if c20 and c126 else None; out['f148_generated']=bool(s148)
 ...
 c17=cover(W['f17']); out['f17_cover']=bool(c17); s258=cp(c17,s244,W['f258']) if c17 and s244 else None; out['f258_generated']=bool(s258); out['f258_seed_added']=False
 if s258: out['f258_seed_added']=bool(e.search.add_clause(s258))
 found=e.solve() if s258 else None; out['seeded_found']=bool(found); out['seeded_replay_ok']=False; out['proof_nodes']=None
 if found:
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_F258_ATTACHMENT',json.dumps(out,sort_keys=True),flush=True)
