...
def main():
 ...
 m=load(SOLVER,'mg231'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h231')
 ...
 W=h.extract_wanted(proof,target[2],m,('f18','f19','f20','f27','f81','f95','f123','f126','f130','f148','f150','f196','f217','f229','f231'))
 def cover(eq):
 ...
 s231=cp(s229,c126,W['f231']) if s229 and c126 else None; out['f231_generated']=bool(s231); out['f231_seed_added']=False
 if s231: out['f231_seed_added']=bool(e.search.add_clause(s231))
 found=e.solve() if s231 else None; out['seeded_found']=bool(found); out['seeded_replay_ok']=False; out['proof_nodes']=None
 if found:
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_F231_ATTACHMENT',json.dumps(out,sort_keys=True),flush=True)
