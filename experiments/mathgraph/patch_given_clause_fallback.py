...
replacement='''    if finish_target_grounded_candidate(\n        source, target, grounded_search, grounded_found\n    ):\n        return\n\n    # Promotion candidate: same generic given-clause controller that uniquely\n    # closed hard_0196 and whose generated certificate passed the official Lean\n    # judge. It is deliberately late so all existing 795 routes are unchanged.\n    if run_given_clause_fallback(source, target, timeout):\n        return\n\n    stair_seconds = min(2.0, max(0.1, timeout / 50.0))\n'''
