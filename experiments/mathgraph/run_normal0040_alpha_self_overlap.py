...
HELPER=ROOT/'experiments/mathgraph/run_normal0040_materialized_self_overlap_original.py'
...
def main():
    ...
    m=load(SOLVER,'mg_alpha_solver'); h=load(HELPER,'mg_alpha_helper')
    ...
    for block in h.fof_blocks(proof):
        ...
        if kind=='definition':
            ...
            continue
        if fid in ('f81','f95'): wanted[fid]=(h.map_rigids(h.inline_defs(x,defs),target[2]),h.map_rigids(h.inline_defs(y,defs),target[2]))
    f81,f95=wanted['f81'],wanted['f95']; cover=None; cover_map=None; cover_rev=False
    for c in engine.search.clauses:
        ...
        for rev,(u,v) in enumerate(((x,y),(y,x))):
            subst={}
            if rigid.match_term(u,f81[0],subst) and rigid.match_term(v,f81[1],subst): cover=(c,x,y); cover_map=subst; cover_rev=bool(rev); break
        if cover: break
    ...
    if cover:
        ...
        base=c if not cover_rev else m.Recipe(c.rhs,c.lhs,'symmetry',(c,)); mat=engine.search.instantiate(base,cover_map)
        for path in rigid.nonvariable_positions(mat.lhs,maximum_depth=limits['maximum_depth'],include_root=True):
            ...
            ix=h.inline_engine_names(p.lhs,engine.reverse_constants); iy=h.inline_engine_names(p.rhs,engine.reverse_constants); same=alpha_sig(rigid,ix,iy)==alpha_sig(rigid,f95[0],f95[1])
            out['proposals'].append({'path':list(path),'lhs':m.render_term(ix),'rhs':m.render_term(iy),'alpha_match_f95':same})
            if same:
