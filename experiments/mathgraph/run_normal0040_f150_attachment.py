...
def main():
 ...
 m=load(SOLVER,'mg150'); h=load(Path(__file__).with_name('run_normal0040_alpha_helpers.py'),'h150')
 ...
 c130=cover(W['f130']); out['f130_cover']=bool(c130); s150=cp(c130,c130,W['f150']) if c130 else None; out['f150_generated']=bool(s150); out['f150_seed_added']=False
 if s150: out['f150_seed_added']=bool(e.search.add_clause(s150))
 found=e.solve() if s150 else None; out['seeded_found']=bool(found); out['seeded_replay_ok']=False; out['proof_nodes']=None
 if found:
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('NORMAL0040_F150_ATTACHMENT',json.dumps(out,sort_keys=True),flush=True)
