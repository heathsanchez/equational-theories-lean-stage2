...
def run_one(rid,input_path,m,h):
    ...
    out={'id':rid,'warm_found':bool(warm),'oracle_free':True,'policy':'fair-pair-coverage-before-expansion','frozen_from_0040':True}
    if warm:
...
def main():
    ...
    rows=[]
    for rid in ids:
        inp=a.normal_input if rid.startswith('evaluation_normal_') else a.hard_input
        res=run_one(rid,inp,m,h); rows.append(res); print('SIX_TRUE_FAIR_PAIR_CASE',json.dumps(res,sort_keys=True),flush=True)
    out={'policy':'fair-pair-coverage-before-expansion','frozen_from_0040':True,'excluded_calibration_case':EXCLUDE,'attempted':len(rows),'solved':sum(bool(x.get('found')) for x in rows),'replay_ok':sum(bool(x.get('replay_ok')) for x in rows),'rows':rows}
    Path(a.output).parent.mkdir(parents=True,exist_ok=True); Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print('SIX_TRUE_FAIR_PAIR_TRANSFER',json.dumps({k:out[k] for k in ('attempted','solved','replay_ok')},sort_keys=True),flush=True)
