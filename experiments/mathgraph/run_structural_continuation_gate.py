...
OUT = Path(__file__).parent / "results/structural-continuation-gate.json"
WINDOWS = (0, 1, 2, 4)
RATIO = 7
...
def collect_vars(t, out):
    ...
    if isinstance(t, tuple):
        if t and t[0] in ("var", "v"):
            ...
            return
        if len(t) >= 3:
            collect_vars(t[1], out)
            collect_vars(t[2], out)
            return
    for left, right in (("left", "right"), ("lhs", "rhs")):
        if hasattr(t, left) and hasattr(t, right):
            collect_vars(getattr(t, left), out)
            collect_vars(getattr(t, right), out)
            return
    if hasattr(t, "args"):
        for x in t.args:
            collect_vars(x, out)
...
def clause_var_count(c):
    vs = set()
    collect_vars(c.lhs, vs)
    collect_vars(c.rhs, vs)
    return len(vs)
...
def one(m, r, window, seconds):
    ...
    proposals_total = accepted_total = gate_pass_total = 0
    ...
    while passive and given < 1024 and not eng.search.expired():
        ...
        if cand:
            idx = min(cand, key=lambda i: (eng.search.target_score(passive[i]), age.get(ckey(m, passive[i]), 10**18)))
            ...
            idx = min(range(len(passive)), key=lambda i: (eng.search.target_score(passive[i]), age.get(ckey(m, passive[i]), 10**18)))
            target_picks += 1
                ...
                for oside, outer in enumerate(base.oriented_variants(m, bo)):
                    for iside, inner in enumerate(base.oriented_variants(m, bi)):
                        for path in m.nonvariable_positions(outer.lhs, maximum_depth=eng.search.limits["maximum_depth"], include_root=True):
                            ...
                            reduced_size = term_size(qr.lhs) + term_size(qr.rhs)
                            vars_n = clause_var_count(qr)
                            k = ckey(m, qr)
                            passes = raw_size <= parent_size + 1 and vars_n <= 3 and reduced_size < parent_size
                            gate[k] = gate.get(k, False) or passes
...
def main():
    ...
    out = {
        "ratio": RATIO,
        "windows": WINDOWS,
        "gate": "raw_size <= parent_size+1 AND distinct_vars <= 3 AND reduced_size < parent_size",
        "residuals": [],
        "protected": [],
    }
    try:
        for rid in RESIDUAL:
            ...
            print("STRUCTURAL_GATE_RESIDUAL", json.dumps(rec, sort_keys=True), flush=True)
        ...
        for rid in PROTECTED:
            ...
            print("STRUCTURAL_GATE_PROTECTED", json.dumps(rec, sort_keys=True), flush=True)
        ...
        out["summary"] = summary
        print("STRUCTURAL_GATE_SUMMARY", json.dumps(summary, sort_keys=True), flush=True)
    finally:
        td.cleanup()
